...
document.getElementById("uploadForm").addEventListener("submit", function (e) {
  ...
  const umur = this.umur.value + " Bulan";
  ...
  if (fileInput.files && fileInput.files[0]) {
    ...
    reader.onload = function (event) {
      ...
      card.className = "card";
      card.innerHTML = `
        <img src="${imgURL}" alt="Foto ${nama}">
        <h3>${nama}</h3>
        <p><strong>Umur:</strong> ${umur}</p>
        <p><strong>Kondisi:</strong> ${kondisi}</p>
        <p><strong>Alamat:</strong> ${alamat}</p>
        <button onclick="bukaForm('${nama}')">Adopsi</button>
      `;
      container.appendChild(card);

      // Reset form setelah berhasil upload
      e.target.reset();
      alert("Kucing berhasil diupload!");
    };

    reader.readAsDataURL(fileInput.files[0]);
  } else {
    alert("Silakan pilih gambar kucing.");
  }
});
...
const form = document.getElementById('reviewForm');
    ...
    // Ambil review dari localStorage saat halaman dimuat
    document.addEventListener('DOMContentLoaded', function () {
      const savedReviews = JSON.parse(localStorage.getItem('reviews')) || [];
      savedReviews.forEach(review => tampilkanReview(review));
    });

    // Fungsi untuk menampilkan review di halaman
    function tampilkanReview(review) {
      const div = document.createElement('div');
      div.className = 'review-item';
      div.innerHTML = `<strong>${review.nama}</strong><p>${review.isi}</p>`;
    ...
    form.addEventListener('submit', function (e) {
      ...
      const review = { nama, isi };

      const existingReviews = JSON.parse(localStorage.getItem('reviews')) || [];
      existingReviews.push(review);
      localStorage.setItem('reviews', JSON.stringify(existingReviews));

      tampilkanReview(review);
      form.reset();
    });